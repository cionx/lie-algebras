...
\end{prop}
\begin{proof}
 Let $S \coloneqq \g_\alpha \oplus k\alpha^\vee \oplus \g_{-\alpha}$. Because $[\g_\alpha, \g_{-\alpha}] = k\alpha^\vee$ there exist $\hat{e} \in \g_\alpha$ and $\hat{f} \in \g_{-\alpha}$ with $[\hat{e},\hat{f}] = \alpha^\vee$. Because $\alpha^\vee \neq 0$ it follows that $\hat{e}, \hat{f} \neq 0$. Also notice that $[\alpha^\vee, \hat{e}] = \alpha(\alpha^\vee) \hat{e} = 2\hat{e}$ and similary $[\alpha^\vee, \hat{f}] = -2\hat{f}$. It follows that $S$ is a Lie subalgebra of $\g$ and
 \[
  ...
  f \mapsto \hat{f}
 \]
 an isomorphism of Lie algebras. Therefore $\g$ becomes a representation of $\sll_2(k)$ via $x.y = [\phi(x), y]$ for every $x \in \sll_2(k)$ and $y \in \g$. Notice that
 \[
 ...
 From this claim it follows that $\alpha/2$ is not root, so $L_1 = \g_{\alpha/2} = 0$. Hence $L$ contains no summand with odd weights and therefore already $L = S$. As a direct consequence $\dim \g_\alpha = \dim L_2 = \dim S_2 = 1$ and $k\alpha \cap \Phi = \{c \in k \setminus \{0\} \mid L_{2c} \neq 0\} = \{1,-1\}$.
\end{proof}
