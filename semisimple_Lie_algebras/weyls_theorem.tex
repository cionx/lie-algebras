...
\begin{proof}
 ...
 \begin{center}
  \tikzsetnextfilename{proof_of_weyls_theorem}
  \begin{tikzpicture}[node distance = 5em]
   \node (kVU) {$\Hom_k(V,U)$};
   \node[right = 5em of kVU] (kUU) {$\Hom_k(U,U)$};
   \node[below of = kVU] (gVU) {$\Hom_k(V,U)^\g$};
   \node[below of = kUU] (gUU) {$\Hom_k(U,U)^\g$};
   \draw[->] (kVU) to node[above] {$\rho$} (kUU);
   \draw[->] (gVU) to node[below] {$\bar{\rho}$} (gUU);
   \draw[->] (kVU) to node[left] {$\pi_V$} (gVU);
   \draw[->] (kUU) to node[right] {$\pi_U$} (gUU);
  \end{tikzpicture}
 \end{center}
