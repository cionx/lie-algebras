...
\begin{prop}
 For all $n,m \in \N$ let $V^{(n)}$ denote the ($n+1$)-dimensional irreducible representation of $\sll_2(k)$. Then
 \[
...
\begin{proof}
 ...
 \[
  (V \otimes W)_i
  = \bigoplus_{\substack{p = 0, \dotsc, n \\ q = 0, \dotsc, m \\ -(n+m) + 2(p+q) = i}} V_{-n+2p} \otimes W_{-m+2q}
  \quad\text{for every $i \in \Z$}.
 \]
 and hence for every $i \in \Z$
 \begin{align*}
  \dim (V \otimes W)_i
  &= |\{(p,q) \in \{0, \dotsc, n\} \times \{0, \dotsc, m\} \mid -(n+m)+2(p+q) =  i\}| \\
  &= |\{(p,q) \in \{0, \dotsc, n\} \times \{0, \dotsc, m\}  \mid 2(p+q) =  n+m+i\}|
 \end{align*}
 To count those dimensions notice that the equality
 \begin{equation}\label{eqn: clebsch gordan by counting}
  2(p+q) = n+m+i \quad \text{with $p \in \{0, \dotsc, n\} \times \{0, \dotsc, m\}$}
 \end{equation}
 has no solution if $n+m+i$ is odd, if $i < -(n+m)$ or if $i > n+m$. So it is enough to count the solutions of \eqref{eqn: clebsch gordan by counting} for $i = -(n+m), -(n+m)+2, \dotsc, n+m-2, n+m$. The solutions are given in the table in figure \ref{fig: solutions for counting dimensions}.
 \begin{figure}
 \[
  \begin{array}{c|c|c}
  i      & \text{solutions} & \begin{tabular}{c} number \\ of \\ solut. \end{tabular} \\
  \hline
  n+m    & (n,m)                                             & 1      \\
  n+m-2  & (n-1, m), (n, m-1),                               & 2      \\
  n+m-4  & (n-2, m), (n-1, m-1), (n, m-2),                   & 3      \\
  \vdots & \vdots                                            & \vdots \\
  n-m    & (n-m, m), (n-m+1, m-1), \dotsc, (n-1, 1), (n,0)   & m      \\
  n-m-2  & (n-m-1,m), (n-m, m-1), \dotsc, (n-2, 1), (n-1, 0) & m      \\
  \vdots & \vdots                                            & \vdots \\
  -n+m+2 & (m+1,0), (m,1), \dotsc, (2,m-1), (1,m)            & m      \\
  -n+m   & (m,0), (m-1, 1), \dotsc, (1, m-1), (0,m)          & m      \\
  \vdots & \vdots                                            & \vdots \\
  -n-m+4 & (2,0), (1,1), (0,2)                               & 3      \\
  -n-m+2 & (1,0), (0,1)                                      & 2      \\
  -n-m   & (0,0)                                             & 1
