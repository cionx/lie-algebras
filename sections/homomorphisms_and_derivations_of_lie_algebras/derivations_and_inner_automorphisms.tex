...
\begin{definition}
	...
	Let~$\Aalg$ be a~\enquote{\algebra{$\kf$}}.
	A \defemph{derivation} of~$\Aalg$ is a~\linear{$\kf$} map~$\delta$ from~$\Aalg$ to~$\Aalg$ such that
	\[
		\delta(ab)
		=
		\delta(a) b + a \delta(b)
	...
	\]
	The set of derivations of~$\Aalg$ is denoted by
	\[
		\Der(\Aalg)
		\defined
		\{
			...
			\Aalg
		\suchthat
			\text{$\delta$ is a derivation of~$\Aalg$}
		\}  \,.
		\glsadd{derivations}
	\]
\end{definition}
...
\begin{proposition}
	Let~$\Aalg$ be a~\enquote{\algebra{$\kf$}}.
	The set~$\Der(\Aalg)$ is a Lie~subalgebra of~$\gllie(\Aalg)$.
\end{proposition}


\begin{proof}
	For any two element~$a$,~$b$ of~$\Aalg$, the auxiliary map
	\[
		h_{ab}
		\colon
		\gllie(\Aalg)
	...
	\[
		...
		\ker( h_{ab} )
	\]
	is a linear subspace of~$\gllie(\Aalg)$.
	If~$\delta_1$ and~$\delta_2$ are any two derivations of~$\Aalg$, then
	\[
		\delta_1( \delta_2(ab) )
		=
	...
	\]
	for all~$a, b \in \Aalg$, and therefore
	\begin{align*}
		[\delta_1, \delta_2](ab)
		&=
		...
		[\delta_1, \delta_2](a) b + a [\delta_1, \delta_2](b) \,.
	\end{align*}
	This shows that the commutator~$[\delta_1, \delta_2]$ of any two derivations~$\delta_1$ and~$\delta_2$ of~$\Aalg$ is again such a derivation.
	This means that the linear subspace~$\Der(\Aalg)$ of~$\gllie(\Aalg)$ is indeed a Lie~subalgebra.
\end{proof}
...
\begin{lemma}
	...
	Let~$\Aalg$ be an~\enquote{\algebra{$\kf$}} and let~$\delta$ be a derivation of~$\Aalg$.
	\begin{enumerate}
		\item
			If~$\Aalg$ is unital, then~$\delta(1) = 0$.
		\item
			The set~$\Aalg' \defined \{ a \in A \suchthat \delta(a) = 0 \}$ is a subalgebra of~$\Aalg$.
			If~$\Aalg$ is unital, then~$\Aalg'$ is a unital subalgebra in the sense that~$1$ is contained in~$\Aalg'$.
	\end{enumerate}
\end{lemma}
...
\begin{definition}
 ...
 Let~$\glie$ be a Lie~algebra.
 A derivation of~$\glie$ is \defemph{inner}\index{derivation!inner}\index{inner derivation!of a Lie algebra} if it is of the form~$\ad(x)$ for some element~$x$ of~$\glie$.
...
\begin{definition}
	Let~$\glie$ be a Lie~algebra.
	The quotient Lie~algebra~$\Der(\glie) / {\InnDer(\glie)}$ is the Lie~algebra of \defemph{outer derivations}\index{derivation!outer}\index{outer derivation!of a Lie algebra} of~$\glie$.
...
\begin{lemma}
	Let~$\Aalg$ be a~\enquote{\algebra{$\kf$}} and let~$\delta$ be a derivation of~$\Aalg$.
	Then
	\[
		\delta^n(ab)
		=
		...
		\qquad
		\text{for all~$n \geq 0$ and~$a, b \in \Aalg$.}
	\]
\end{lemma}
...
\begin{proof}[Second proof]
	...
	\[
		...
		ab
	\]
	be the multiplication map of~$\Aalg$.
	That~$\delta$ is a derivation of~$\Aalg$ means that
	\[
		\delta \circ m
		=
		m \circ (\delta \tensor \id + \id \tensor \delta) \,.
	\]
	The endomorphism~$\delta \tensor \id$ and~$\id \tensor \delta$ of~$\Aalg \tensor \Aalg$ commute.
...
\begin{proposition}
	...
	Let~$\Aalg$ be a~\enquote{\algebra{$\kf$}} where~$\kf$ is a field of characteristic zero and let~$\delta$ be a nilpotent derivation of~$\Aalg$.
	Then~$\exp(\delta)$ is an algebra automorphism of~$\Aalg$.
\end{proposition}


\begin{proof}
	Let~$\alpha$ be~$\exp(\delta)$.
	We have for any two element~$a$,~$b$ of~$\Aalg$ that
	\begingroup
	\allowdisplaybreaks
	\begin{align*}
		\alpha(ab)
