...
\begin{definition}
	...
	A \defemph{grading} or \defemph{gradation} of~$A$ is a direct sum decomposition
	\[
		A
		=
	...
	\]
	for all~$p, q \geq 0$.
	A \defemph{graded~\algebra{$\kf$}} is a~\algebra{$\kf$}~$A$ together with a grading~$A = \bigoplus_{p \geq 0} A_p$.
\end{definition}
...
\begin{remark}
	...
	An~\defemph{\grading{$S$}} of a~\algebra{$\kf$}~$A$ is a direct sum decomposition
	\[
		A
		=
		\bigoplus_{s \in S}
...
\begin{definition}
	...
	\begin{enumerate}
		\item
			An element~$x$ of~$A$ is \defemph{homogeneous} of \defemph{degree}~$p$ if~$x$ is contained in the direct summand~$A_p$.
		\item
			...
			The summands~$x_p$ are the~\defemph{homogeneous components} of~$x$, and the decomposition~$x = \sum_{p \geq 0} x_p$ is the~\defemph{homogeneous decomposition} of~$x$.
	\end{enumerate}
\end{definition}
...
\begin{definition}
	...
	\begin{enumerate}
		\item
			A \defemph{homomorphism of graded~\algebras{$\kf$}} from~$A$ to~$B$ is a homomorphism of~\algebras{$\kf$}~$\Phi$ from~$A$ to~$B$ such that the image~$\Phi(A_p)$ is contained in the homogeneous part~$B_p$ for every~$p \geq 0$.
		\item
...
\begin{definition}
	Let~$A$ be a graded~\algebra{$\kf$}.
	An ideal~$I$ of~$A$ (of any kind) is \defemph{homogeneous} if it satisfies the equivalent conditions from \cref{characterizations of homogeneous ideals}.
