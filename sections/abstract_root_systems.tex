...
\begin{definition}
  A \defemph{reflection}\index{reflection} in a vector space~$V$ is a map~$s \colon V \to V$ with~$s^2 = \id_V$ and such that the fixed subspace~$\{v \in V \suchthat s(v) = v\}$ has codimension~one.
\end{definition}
...
\begin{definition}
  A root system~$R$ is a vector space~$V$ is \defemph{reducible}\index{reducible root system}\index{root system!reducible}%
...
% \begin{example}[External union of root systems]
%   Let~$V_1, \dotsc, V_n$ be finite dimensional vector spaces, and for every~$i = 1, \dotsc, n$ let~$R_i$ be a root system in~$V_i$.
%   The~$R_i$ assemble to a root system~$R$ of~$V \defined V_1 \oplus \dotsc \oplus V_n$:
%   
%   We can regard every~$V_i$ as a subspace of~$V$, and hence every~$\alpha \in R_i$ as an element of~$V$.
%   In~$V$ we can thus consider the disjoint union~$R \defined R_1 \dcup \dotsb \dcup R_n$.
...
%   \]
%   
%   Then~$R$ is a root system in~$V$ with coroots~$\check{R} = \check{R_1} \dcup \dotsb \dcup \check{R_n}$:
%   
%   Every~$R_i$ spans~$V_i$ whence~$R$ spans~$V$.
%   For every~$\alpha \in R$ with~$\alpha \in R_i$ the coroot~$\check{\alpha}$, when regarded as an element of~$V^*$, satisfies~$\pair{\alpha, \check{\alpha}} = 2$ and~$\pair{\beta, \check{\alpha}} = 0$ for every~$\beta \in R_j$ with~$j \neq i$.
%   Then~$s_{\alpha, \check{\alpha}}(\beta) = \beta$ for every~$\beta \in R_j$ with~$i \neq j$, and~$s_{\alpha, \check{\alpha}}(R_i) = R_i$.
%   Thus~$R$ is a root system as claimed.
% \end{example}
...
\begin{definition}
  For a root system~$R$ in a vector space~$V$ its \defemph{Weyl~group}\index{Weyl group}~$\Weyl(R)$ is the subgroup of~$\GL(V)$ generated by all reflections~$s_\alpha$ with~$\alpha \in R$.
\end{definition}
