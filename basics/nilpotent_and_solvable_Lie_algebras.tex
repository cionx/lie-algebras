...
\end{lem}
\begin{proof}
 For $z \in g$ and $y \in I^\perp$ it follows for every $x \in I$ that
 \[
  \beta(x,[y,z]) = \beta(\underbrace{[x,y]}_{=0},z) = 0
 \]
 and thus $[y,z] \in I^\perp$.
\end{proof}


\begin{rem}
 The proof of Lemma \ref{lem: orthogonal complement of an ideal is again an ideal} did not use that $\beta$ is symmetric. This artficial restraint is only there to simplify the situation and notation (we do not need to distinguish between orthogonal complements from the left and from the right.) As the main example of an associative bilinear form will be the Killing form, which is symmetric, this simplification will pose no problems to us.
\end{rem}
