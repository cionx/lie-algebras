...
\begin{defi}
 Let $\g$ be a $k$-Lie algebra. A \emph{representation} of $\g$ is a $k$-vector space $V$ together with a homomorphism of Lie algebras $\rho \colon \g \to \gl(V)$. $\rho$ is called \emph{faithful} if it is injective.
\end{defi}
...
\begin{prop}
 ...
 \begin{enumerate}[leftmargin=*]
  \item
   ...
   \[
    V^* \oplus W \to \Hom_k(V,W), \varphi \otimes w \mapsto (v \mapsto \varphi(v) w)
   \]
  ...
  \item
   ...
   \end{align*}
   is already an isomorphism of Lie algebras.
  \item
